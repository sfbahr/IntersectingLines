...
public class BPTreeTest extends TestCase
{
    ...
    /**
     * This method tests the list method from BPTree,
     * lists the Key/Value pairs in th correct locations.
     */
    public void testList()
    {
        tree.insert(new KVPair(new Handle(9), new Handle(0)));
        tree.insert(new KVPair(new Handle(9), new Handle(1)));
        tree.insert(new KVPair(new Handle(9), new Handle(2)));
        tree.insert(new KVPair(new Handle(9), new Handle(3)));
        tree.insert(new KVPair(new Handle(9), new Handle(4)));
        tree.insert(new KVPair(new Handle(10), new Handle(5)));
        tree.insert(new KVPair(new Handle(11), new Handle(6)));
        DoublyLinkedList<Handle> matches = tree.list(new Handle(9));
        DoublyLinkedList<Handle> correct = new DoublyLinkedList<Handle>();
        correct.insertAfterCurrent(new Handle(0)).next();
        correct.insertAfterCurrent(new Handle(1)).next();
        correct.insertAfterCurrent(new Handle(2)).next();
        correct.insertAfterCurrent(new Handle(3)).next();
        correct.insertAfterCurrent(new Handle(4)).next();
        assertEquals(matches.getCurrent().get(), correct.getCurrent().get());
        matches.previous();
        correct.previous();
        assertEquals(matches.getCurrent().get(), correct.getCurrent().get());
        matches.previous();
        ...
        correct.previous();
        assertEquals(matches.getCurrent().get(), correct.getCurrent().get());
        assertEquals(matches.getSize(), correct.getSize());
        tree.print();

        tree = new BPTree();
        tree.insert(new KVPair(new Handle(8), new Handle(6)));
        tree.insert(new KVPair(new Handle(9), new Handle(0)));
        tree.insert(new KVPair(new Handle(9), new Handle(1)));
        tree.insert(new KVPair(new Handle(9), new Handle(2)));
        tree.insert(new KVPair(new Handle(9), new Handle(3)));
        tree.insert(new KVPair(new Handle(9), new Handle(4)));
        tree.insert(new KVPair(new Handle(10), new Handle(5)));
        matches = tree.list(new Handle(9));
        correct = new DoublyLinkedList<Handle>();
        correct.insertAfterCurrent(new Handle(0)).next();
        correct.insertAfterCurrent(new Handle(1)).next();
        correct.insertAfterCurrent(new Handle(2)).next();
        correct.insertAfterCurrent(new Handle(3)).next();
        ...
        tree = new BPTree();
        tree.insert(new KVPair(new Handle(7), new Handle(6)));
        tree.insert(new KVPair(new Handle(8), new Handle(6)));
        tree.insert(new KVPair(new Handle(9), new Handle(0)));
        tree.insert(new KVPair(new Handle(9), new Handle(1)));
        tree.insert(new KVPair(new Handle(9), new Handle(2)));
        ...
        tree.insert(new KVPair(new Handle(5), new Handle(5)));
        tree.insert(new KVPair(new Handle(7), new Handle(6)));
        tree.insert(new KVPair(new Handle(9), new Handle(0)));
        tree.insert(new KVPair(new Handle(9), new Handle(1)));
        tree.insert(new KVPair(new Handle(9), new Handle(2)));
        ...
        tree.insert(new KVPair(new Handle(9), new Handle(3)));
        tree.insert(new KVPair(new Handle(9), new Handle(4)));
        tree.insert(new KVPair(new Handle(9), new Handle(5)));
        tree.insert(new KVPair(new Handle(9), new Handle(6)));
        tree.insert(new KVPair(new Handle(9), new Handle(7)));
        matches = tree.list(new Handle(9));
        correct = new DoublyLinkedList<Handle>();
        correct.insertAfterCurrent(new Handle(0)).next();
        correct.insertAfterCurrent(new Handle(1)).next();
        ...
        correct.insertAfterCurrent(new Handle(3)).next();
        correct.insertAfterCurrent(new Handle(4)).next();
        correct.insertAfterCurrent(new Handle(5)).next();
        correct.insertAfterCurrent(new Handle(6)).next();
        correct.insertAfterCurrent(new Handle(7)).next();
        assertEquals(matches.getCurrent().get(), correct.getCurrent().get());
        matches.previous();
        correct.previous();
        assertEquals(matches.getCurrent().get(), correct.getCurrent().get());
